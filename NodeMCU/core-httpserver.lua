		...
		coroutine.yield()
         ...
         local function onRequest(connection, req)
            ...
            if #(uri.file) > 32 then
               ...
               fileServeFunction = dofile("core-httpserver-error.lc")
            else
               ...
               elseif uri.isScript then
                  ...
                  if allowStatic[method] then
                    ...
                    fileServeFunction = dofile("core-httpserver-static.lc")
                  else
         ...
         local function onReceive(connection, payload)
            collectgarbage()
            local conf = dofile("core-httpserver-conf.lc")
            local auth
            local user = "Anonymous"

            -- parse payload and decide what to serve.
            local req = dofile("core-httpserver-request.lc")(payload)
            print("Requested URI: " .. req.request)
            if conf.auth.enabled then
               auth = dofile("core-httpserver-basicauth.lc")
            ...
            if user and req.methodIsValid and (req.method == "GET" or req.method == "POST" or req.method == "PUT") then
               ...
               local args = {}
               local fileServeFunction = dofile("core-httpserver-error.lc")
               if not user then
